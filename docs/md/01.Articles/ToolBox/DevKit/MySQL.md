...
update_time: 2025/03/12 21:40
---

## 下载 & 安装

> [!info]
> 安装 MySQL 之前，确保电脑上原来安装的 MySQL 已经彻底卸载干净（包括应用、数据、注册表和系统环境变量等信息，如果是第一次安装的话，则可以不用管）！可以使用原生的 MySQL Installer 或者第三方卸载工具（如[软件卸载工具](../Tools/软件卸载工具.md)中推荐的 [HiBit Uninstaller](../Tools/软件卸载工具.md#^6ce691) 👍 推荐使用这种方式）进行卸载。
